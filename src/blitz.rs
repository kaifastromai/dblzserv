...
mod tests {
    ...
    #[test]
    fn test_gen_cards() {
        let cards = generate_all_card(4);
        assert_eq!(cards.len(), 160);
    }
    #[test]
    fn test_arena() -> anyhow::Result<()> {
        let cards = generate_all_card(4);
        let mut card_context = CardContext::new(cards);
        let mut arena = Arena::new();
        //add all the cards to the arena
        for (i, _card) in card_context.cards.clone().iter().enumerate() {
            arena
                .add_card((i / 10) as u8, i as u32, &mut card_context)
                .with_context(|| format!("Failed to add card {}", i))?;
        }

        Ok(())
    }
    #[test]
    fn test_game_state() -> anyhow::Result<()> {
        let cards = generate_all_card(4);
        let _card_context = CardContext::new(cards);
        let game_state = GameStateBuilder::new().build()?;

        for player in game_state.players.iter() {
            //should have 27 cards in hand
            assert_eq!(player.hand.count_in_hand(), 27);
            //should have 10 cards in the blitz pile
            assert_eq!(player.blitz_pile.cards.len(), 10);
            //should have 3 post piles
            assert_eq!(player.post_pile.piles.len(), 3);
        }
