...
use crate::{
    proto::{ArenaPlayType, SessionRes},
    *,
};
...
impl Session {
    ...
    async fn notify_players_pause_game(&mut self, player: &proto::Player) -> Result<()> {
        for pl in self.players.iter_mut() {
            if player.id != pl.player.id {
                pl.connection.pause_game(tonic::Request::new(())).await?;
            }
        }
        Ok(())
    }
    async fn notify_players_resume_game(&mut self, player: &proto::Player) -> Result<()> {
        for pl in self.players.iter_mut() {
            if player.id != pl.player.id {
                pl.connection.resume_game(tonic::Request::new(())).await?;
            }
        }
        Ok(())
    }
...
impl proto::session_service_server::SessionService for SessionMan {
    ...
    ) -> Result<tonic::Response<proto::SessionRes>, Status> {
        info!("Getting active sessions!");
    ...
    async fn join_session(
        ...
        if !session.is_joinable {
            return Err(Status::failed_precondition("session is not joinable"));
        }
    ...
    ) -> Result<tonic::Response<proto::Player>, Status> {
        info!("Starting sessions");
        let rq = request.into_inner();
    ...
    async fn start_game(
        ...
        // let crate_deduction = prefs.crate_deduction as u16;
        let player_count = session.players.len() as u8;
        //start the game
        let game_state =
            crate::GameState::new(draw_rate, post_pile_size, player_count, score_to_win, 10)
                .unwrap();
            ...
            proto::play::Play::PlayerPlay(p) => {
                ...
                let player_play = match proto::PlayerPlayType::from_i32(p.play_type).unwrap() {
                    proto::PlayerPlayType::BlitzToPost => {
                        crate::PlayerPlay::BlitzToPost(p_play_val)
                    }
                    proto::PlayerPlayType::AvailableHandToPost => {
                        crate::PlayerPlay::AvailableToPost(p_play_val)
                    }
                    proto::PlayerPlayType::TransferToAvailableHand => {
                        crate::PlayerPlay::TransferToAvailable
                    }
                    proto::PlayerPlayType::ResetHand => crate::PlayerPlay::ResetHand,
                };
                EPlay::PlayerPlay(player_play)
            }
            proto::play::Play::CallBlitz(p) => {
                let crate_val = p.player_index as u8;
                crate::EPlay::CallBlitz(crate_val)
            }
        };
...
impl proto::game_service_server::GameService for SessionMan {
    async fn make_play(
        ...
        let ses_id = ses_id.map_err(|e| Status::invalid_argument(e.to_string()))?;
